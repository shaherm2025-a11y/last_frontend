...
Future<int?> ensureAutoLogin() async {
  ...
  try {
    ...
    final uri = Uri.parse('https://mohashaher-plant-diag-final-server.hf.space/auto_login'); // √Ê «” »œ· »⁄‰Ê«‰ Œ«œ„ﬂ
...
void main() async {
  ...
  if (kIsWeb) {
	...
	//  ÂÌ∆… SQLite + «” Ì—«œ JSON ≈–« ﬂ«‰  «·ﬁ«⁄œ… ›«—€…
    await DatabaseHelper.getDatabase();
  }
   // ====== Android / iOS ======
  else {
    await DatabaseHelper.getDatabase(); // Â–« Ì‘€¯· «·≈‰‘«¡ + «·«” Ì—«œ
  }
...
//
  ...
  Future<void> diagnoseAndSave(Uint8List imageBytes, String filename) async {
    ...
    try {
      ...
      final uri = Uri.parse('https://mohashaher-plant-diag-final-server.hf.space/predict');
      ...
      final saveUri = Uri.parse('https://mohashaher-plant-diag-final-server.hf.space/save_diagnosis');
  ...
  Future<List<Map<String, dynamic>>> fetchPreviousDiagnoses() async {
    ...
    final uri = Uri.parse('https://mohashaher-plant-diag-final-server.hf.space/previous_diagnoses/$farmerId');
...
class _PestsDiseasesPageState extends State<PestsDiseasesPage> {
  ...
  Future<void> _initializeData() async {
    // «·Â« › ›ﬁÿ: «” Œœ«„ SQLite Ê«” Ì—«œ «·»Ì«‰« 
    if (!kIsWeb) {
      await DatabaseHelper.getDatabase();
    }

    //  Õ„Ì· «·„Õ«’Ì· (SQLite ⁄·Ï «·Â« ›° JSON ⁄·Ï «·ÊÌ»)
    await _loadCrops();
  }
...
class _FarmerQuestionsPageState extends State<FarmerQuestionsPage> {
  ...
  Future<void> _fetchQuestions() async {
    ...
    try {
      final uri = Uri.parse("https://mohashaher-plant-diag-final-server.hf.space/get_farmer_questions/$_farmerId");
      final response = await http.get(uri);
      if (response.statusCode == 200) {
        final List data = json.decode(response.body);
  ...
  Future<void> _sendQuestion() async {
    ...
    try {
      final uri = Uri.parse("https://mohashaher-plant-diag-final-server.hf.space/send_question");
      final request = http.MultipartRequest('POST', uri);
